...
// 理由：上は20+2=22の答えになるが、下の'20'はクォーテーションで囲むと文字列になるので、20の後ろに2が追加され、202が答えになる。
...
// 理由：++が後につく後値では、インクリメント前の値を返すが、前につく前値はインクリメント後の値を返す
...
// 理由：左右の値が等しい場合は、等値演算子の上はtrue、下の同値演算子は厳密な比較をするため値と文字列で処理し比較する為falseとなる
